...
def main():
    ...
    if not st.session_state.interview_started:
        ...
        with col2:
            ...
            cover_letter_file = st.file_uploader("", type=["pdf"], key="cover_letter_uploader")
            if cover_letter_file is not None:
                st.success("Cover letter uploaded ✓")

        # Job Description area
        st.header("Job Description")
        job_description = st.text_area("Paste the job description for the position you are applying for...", value=st.session_state.job_description)
        st.session_state.job_description = job_description
        
        # Check if at least one field is provided
        documents_provided = resume_file is not None or cover_letter_file is not None or job_description.strip()
        
        if not documents_provided:
            st.warning("Please upload at least your resume, cover letter, or provide a job description to proceed.")
        else:
            ...
            with start_col2:
                st.markdown('<div style="background-color: #192841; padding: 10px; border-radius: 5px;">Click to begin your interview session</div>', unsafe_allow_html=True)
    ...
    if hasattr(st.session_state, 'cover_letter_path') and st.session_state.cover_letter_path:
    ...
    if st.session_state.messages[-1]["role"] != "assistant" and st.session_state.total_questions_asked < 3:
    ...
    if (st.session_state.total_questions_asked >= 3 and 
        st.session_state.waiting_for_last_answer and
